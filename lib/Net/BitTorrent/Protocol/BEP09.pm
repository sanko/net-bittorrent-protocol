package Net::BitTorrent::Protocol::BEP09 v1.5.3 {
    ...
    #
    sub build_metadata_request ($index) {
        bencode( { piece => $index, msg_type => 0 } );
    }

    sub build_metadata_data ( $index, $data ) {
        bencode( { piece => $index, total_size => length($data), msg_type => 1 } ) . $data;
    }

    sub build_metadata_reject($index) {
        bencode( { piece => $index, msg_type => 2 } );
    }
};
1;
...
The purpose of this extension is to allow clients to join a swarm and complete a download without the need of
downloading a .torrent file first. This extension instead allows clients to download the metadata from peers. It makes
...
Note that there are no parser functions as the packets generated for BEP09 are simple bencoded hashes. Use the
...
Generates an appropriate reply to a L<request query|/"build_metadata_request( $index )">.

=item C<build_metadata_reject( $index, $piece )>

Generates an appropriate reply to a L<request query|/"build_metadata_request( $index )"> if the requested piece is not
available.
