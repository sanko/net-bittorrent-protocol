package Net::BitTorrent::Protocol::BEP44 v1.5.3 {
    ...
    *import      = *import = *Exporter::import;
    ...
    sub build_put_data_reply ($$) {
        my ( $tid, $nid ) = @_;
        return bencode( { t => $tid, y => 'r', r => { id => $nid }, v => $Net::BitTorrent::Protocol::BEP05::v } );
    }

    sub build_get_data_query ($$$) {
        my ( $tid, $nid, $target ) = @_;
        return bencode( { t => $tid, y => 'q', q => 'get', a => { id => $nid, target => $target }, v => $Net::BitTorrent::Protocol::BEP05::v } );
    }
        ...
        return bencode(
            {   t => $tid,
                ...
                r => {
                    id    => $nid,
                    token => $token,
                    ( $values ? ( v => $values ) : () ), ( $nodes ? ( nodes => $nodes ) : () ), ( $nodes6 ? ( nodes6 => $nodes6 ) : () ),

                    # Mutable
                    ( $k ? ( k => $k ) : () ), ( $seq ? ( seq => $seq ) : () ), ( $sig ? ( sig => $sig ) : () )
                },
                v => $Net::BitTorrent::Protocol::BEP05::v
            }
        );
...
Note that all functions require a transaction ID. Queries also require a user generated node ID.
...
Depending on whether the data is mutable or immutable, this function expects the following params:
...
Remember that the protocol expects this to be less than or equal to 1000 bytes.

=back

The following values are optional unless the data being set is mutable:
...
Builds a proper reply to a request to store mutable or immutable data. This is nearly the same as the standard reply in
BEP05.
...
Please use the C<build_error_reply( $tid, $error )> function of Net::BitTorrent::Protocol::BEP05.

The following describes the expanded list of possible error codes:
