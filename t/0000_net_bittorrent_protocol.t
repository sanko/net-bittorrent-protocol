...
is parse_packet( \{} ), undef, 'parse_packet(\\{ }) == undef (requires SCALAR ref)';
my $packet = 'Testing';
...
is parse_packet( \$packet ), undef, 'parse_packet(\\$packet) == undef (where $packet == "\\0\\0\\0\\5\\40\\0\\0\\0F")';
$packet = undef;
is parse_packet( \$packet ), undef, 'parse_packet(\\$packet) == undef (where $packet == undef)';
$packet = '';
is parse_packet( \$packet ), undef, 'parse_packet(\\$packet) == undef (where $packet == "")';
$packet = "\000\000\000\r\cU\000\000\cD\000\000\cD\000\000\000\cA\000\000";
is parse_packet( \$packet ), undef, 'parse_packet(\\$packet) == undef (where $packet == "\\0\\0\\0\\r\\25\\0\\0\\4\\0\\0\\4\\0\\0\\0\\1\\0\\0")';
...
is parse_packet($data), undef,                      'parse_packet($data) == undef (Requires a SCALAR reference)';
...
is(
    ...
    {   packet_length => 96,
        payload       => [
            0, { m => { "ut_pex" => 1, "\xC3\x82\xC2\xB5T_PEX" => 2 }, p => 30, reqq => 30, v => "Net::BitTorrent r0.30", yourip => "\x7F\0\0\1", },
        ],
        payload_length => 91,
        type           => $EXTENDED
    },
...
for ( 1 .. 5 ) {
    shift @original_data;
    is $data, join( '', @original_data ), '   ...was shifted from data.';
    is( parse_packet( \$data ), { packet_length => 4, payload_length => 0, type => $KEEPALIVE }, 'Keepalive...' );
}
shift @original_data;
is $data, join( '', @original_data ), '   ...was shifted from data.';
is( parse_packet( \$data ), { packet_length => 5, payload_length => 0, type => $INTERESTED }, 'Interested...' );
shift @original_data;
is $data, join( '', @original_data ), '   ...was shifted from data.';
is( parse_packet( \$data ), { packet_length => 4, payload_length => 0, type => $KEEPALIVE }, 'Keepalive...' );
shift @original_data;
is $data, join( '', @original_data ), '   ...was shifted from data.';
is( parse_packet( \$data ), { packet_length => 5, payload_length => 0, type => $NOT_INTERESTED }, 'Not interested...' );
shift @original_data;
is $data, join( '', @original_data ), '   ...was shifted from data.';
is( parse_packet( \$data ), { packet_length => 5, payload_length => 0, type => $UNCHOKE }, 'Unchoke...' );
shift @original_data;
is $data, join( '', @original_data ), '   ...was shifted from data.';
is( parse_packet( \$data ), { packet_length => 5, payload_length => 0, type => $CHOKE }, 'Choke...' );
shift @original_data;
is $data, join( '', @original_data ), '   ...was shifted from data.';
is( parse_packet( \$data ), { packet_length => 4, payload_length => 0, type => $KEEPALIVE }, 'Keepalive...' );
shift @original_data;
...
shift @original_data;
is $data, join( '', @original_data ), '   ...was shifted from data.';
is( parse_packet( \$data ), { packet_length => 5, payload_length => 0, type => $HAVE_ALL }, 'Have All...' );
shift @original_data;
is $data, join( '', @original_data ), '   ...was shifted from data.';
is( parse_packet( \$data ), { packet_length => 5, payload_length => 0, type => $HAVE_NONE }, 'Have None...' );
shift @original_data;
is $data, join( '', @original_data ), '   ...was shifted from data.';
