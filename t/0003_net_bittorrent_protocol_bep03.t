...
is build_bitfield('abcdefg'), "\000\000\000\cH\cEabcdefg",
    'build_bitfield(q[abcdefg]) == "\\0\\0\\0\\b\\5abcdefg"';
my $tmp = join(join(time, keys %ENV), %INC);
is build_bitfield($tmp), pack('N', length($tmp) + 1) . "\cE" . $tmp,
    sprintf('build_bitfield($tmp)       == okay (where $tmp =~ qr[^.{%d}$])',
            length $tmp);
...
is build_piece(1, 2, \$tmp), pack('NcN2a*', length($tmp) + 9, 7, 1, 2, $tmp),
    'build_piece(1, 2, \\$tmp)  == okay';
...
is build_port(8555), "\000\000\000\cE\t\000\000!k",
    'build_port(8555)   == "\\0\\0\\0\\5\\t\\0\\0!k"';
is build_port(652145), "\000\000\000\a\t\000\t\363q",
    'build_port(652145) == "\\0\\0\\0\\a\\t\\0\\t\\xF3q"';

# Parsing functions
is parse_handshake(''), undef,
    q[parse_handshake('') == undef (no/not enough data)];
is parse_handshake('Hahaha'), undef,
...
is parse_have(''), undef, q[parse_have('') == undef (no packed index)];
...
is parse_bitfield(''), undef, q[parse_bitfield('') == undef (no data)];
...
is parse_request(''), undef, q[parse_request('') == undef];
is_deeply(
...
is parse_piece(''), undef, q[parse_piece('') == undef];
...
is parse_cancel(''), undef, q[parse_cancel('') == undef];
